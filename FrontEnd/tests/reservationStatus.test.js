...
test('confirmed reservation becomes overdue warning at 1:01 and no-show at 2:00', () => {
  assert.deepEqual(reservationPresentation(booking, at('13:00')), { status: 'Confirmed', warning: '' });
  assert.deepEqual(reservationPresentation(booking, at('13:01')), { status: 'Confirmed', warning: 'Overdue' });
  assert.deepEqual(reservationPresentation(booking, at('13:59')), { status: 'Confirmed', warning: 'Overdue' });
...
test('active sessions and pending cancellation requests do not become no-shows', () => {
  assert.deepEqual(reservationPresentation({ ...booking, status: 'Ongoing' }, at('14:00')), { status: 'Confirmed', warning: 'In use' });
