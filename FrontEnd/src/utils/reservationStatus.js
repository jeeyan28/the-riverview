...
export const RESERVATION_STATUS_FILTERS = ['Pending', 'Confirmed', 'Done', 'Cancelled', 'No Show', 'Rejected'];
...
export function reservationPresentation(booking, now = Date.now()) {
  ...
  if (rawStatus === 'Confirmed' && booking?.cancellationStatus !== 'Requested') {
    ...
    if (window && now >= window.start + MINUTE_MS && now < window.end) return { status: 'Confirmed', warning: 'Overdue' };
  }
  if (rawStatus === 'Ongoing') return { status: 'Confirmed', warning: 'In use' };
