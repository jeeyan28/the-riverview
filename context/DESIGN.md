...
- Live Monitor payment actions follow the money state: fully paid reservations show a compact confirmation with no collection controls, partial sessions show paid and remaining amounts, and unpaid pay-after-play sessions receive the strongest finish warning. Payment cells pair a status badge with the exact collected or due amount.
- Analytics and report date ranges start empty and wait for an explicit service-date selection. Settings change history shows ten entries per page with Previous and Next navigation.
